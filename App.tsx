...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'build' | 'swap' | 'import'>('build');

  // Initialize schedule from hardcoded CSV
  useEffect(() => {
  ...
  const processCsvIntelligence = async () => {
    ...
    try {
      ...
      // Update the current schedule view from the new CSV
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="container mx-auto p-4 max-w-7xl">
        <div className="flex border-b border-slate-200 mb-10 space-x-12">
          <button onClick={() => setActiveTab('build')} className={`pb-4 px-2 font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'build' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>Builder</button>
          <button onClick={() => setActiveTab('swap')} className={`pb-4 px-2 font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'swap' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>Concierge</button>
          <button onClick={() => setActiveTab('import')} className={`pb-4 px-2 font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'import' ? 'border-b-4 border-indigo-600 text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>Import</button>
        </div>
        ...
        {activeTab === 'import' && (
          <div className="max-w-4xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100">
             ...
             <textarea value={csvContent} onChange={(e) => setCsvContent(e.target.value)} placeholder="Paste CSV here..." className="w-full h-96 p-6 border-2 border-slate-100 rounded-3xl font-mono text-[10px] mb-6 focus:ring-4 focus:ring-indigo-50/50 focus:border-indigo-400 outline-none transition-all" />
             <button onClick={processCsvIntelligence} disabled={isAnalyzingCsv || !csvContent} className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-slate-800 disabled:bg-slate-300 transition w-full shadow-2xl shadow-slate-200">{isAnalyzingCsv ? "Extracting Intelligence..." : "Process Data"}</button>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div className="xl:col-span-8 flex flex-col">
              <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 mb-8">
                <h2 className="text-xl font-black mb-6 text-slate-900 tracking-tight">Optimization Center</h2>
                <div className="flex gap-4">
                  <button onClick={handleGenerateSchedule} disabled={isLoading} className="flex-1 bg-indigo-600 text-white font-black px-8 py-5 rounded-2xl hover:bg-indigo-700 transition flex items-center justify-center gap-3 disabled:bg-slate-200 shadow-2xl shadow-indigo-100 uppercase text-xs tracking-widest">{isLoading ? "Crunching Shifts..." : <><SparklesIcon className="w-5 h-5" /> Generate Optimization</>}</button>
